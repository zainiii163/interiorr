...
export default function ProcessGrid({ pillars = [], copy = {} }) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal className="text-center mb-14">
          <span className="text-[#C4795A] font-semibold text-xs uppercase tracking-widest">
            {copy.homeProcessBadge || 'How We Work'}
          </span>
          <h2 className="font-serif text-4xl sm:text-5xl font-bold text-stone-900 mt-2">
            {copy.homeProcessTitle || 'How We Deliver Every Project'}
          </h2>
          {copy.homeProcessBody && (
            <p className="text-stone-500 text-sm mt-3 max-w-2xl mx-auto">{copy.homeProcessBody}</p>
          )}
        </ScrollReveal>
          ...
          {pillars.map((pillar, i) => {
            ...
            return (
              <ScrollReveal key={pillar._id || pillar.title} delay={i * 80}>
                <div className="h-full p-8 bg-stone-50 rounded-3xl border border-stone-200 hover-lift hover:shadow-lg transition-all duration-300">
